...
const setMonthBarWidth = (date, now) => {
	let elapsed = now - date;
	let curMonthLength = new Date(now.getFullYear(), now.getMonth() + 1, 0).getDate();
...
const setDaysCurrent = () => {
	const currDayNum = 1 + Math.floor((curYearPercent * daysContainer.childNodes.length) / 100);
	Array.from({ length: currDayNum }, (element, index) => {
		daysContainer.querySelector(`:nth-child(${index + 1})`).classList.add("passed");
	});
	daysContainer.querySelector(`:nth-child(${currDayNum})`).classList.add("flicker");
};
